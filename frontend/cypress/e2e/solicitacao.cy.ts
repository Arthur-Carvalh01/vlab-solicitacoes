...
describe('Fluxo de Criação de Solicitações', () => {
  ...
  it('deve exigir justificativa quando a prioridade for URGENTE', () => {
    ...
    // Tenta salvar para forçar a validação HTML5 do campo Justificativa
    cy.get('[data-cy="btn-salvar"]').click();

    // Verifica se o modal continua aberto validando se o título persiste
    cy.contains('Nova Solicitação').should('be.visible');
    
    // Captura a tela e salva na pasta cypress/screenshots
    cy.screenshot('teste-cypress-aprovado');
  });
});